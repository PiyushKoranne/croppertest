...
const Create = () => {
	...
	const handleImageLoad = (e) => {
		...
		console.log(chosenTemplateData?.filler?.profileImage_size?.split(" ")?.join("")?.split("px"))
   ...
   */
   function rotateSize(width, height, rotation) {
	...
	const handleImageCropper = (file) => {
		console.log('FILES ->', file);
		setImageDetails({name:file?.name, mime:file?.type});
		...
		if (cropperRef.current) {
			// You can adjust the width and height values as per your requirements.
			const dimensions = userData?.profileImage_size?.split(" ")?.join("")?.split("px");
	...
	const getCropData = () => {
		if (typeof cropperRef.current?.cropper !== "undefined") {
			...
			handleImageUpload(cropperRef.current?.cropper.getCroppedCanvas().toDataURL());
		}
	};

	const handleCropMove = () => {
		console.log('Cropbox Moved - I was called')
		if (cropperRef.current) {
			// getCropData();
			const cropBoxData = cropperRef.current.cropper.getData();
			const cropBoxWidth = cropBoxData.width;
			const cropBoxHeight = cropBoxData.height;
			setCropperDimensions({width:Math.round(cropBoxWidth), height:Math.round(cropBoxHeight)})
		}
	}
			...
			try {
				...
				switch (domNode.attribs && domNode.attribs.id) {
					...
					case ('link-facebook-blk'):
						...
						if (!userData.facebook || !socialChecker?.facebook) {
					...
					case ('link-skype-blk'):
						props = attributesToProps(domNode.attribs);
						if (!userData.skype || !socialChecker?.skype) {
							props.style.display = 'none'
						} else {
							props.style.display = 'inline-block'
						}
					...
					case ('link-whatsapp-blk'):
						props = attributesToProps(domNode.attribs);
						if (!userData.whatsapp || !socialChecker?.whatsapp) {
							props.style.display = 'none'
						} else {
							props.style.display = 'inline-block'
						}
					...
					case ('link-linkedIn-blk'):
						props = attributesToProps(domNode.attribs);
						if (!userData.linkedIn || !socialChecker?.linkedin) {
							props.style.display = 'none'
						} else {
							props.style.display = 'inline-block'
						}
					...
					case ('link-twitter-blk'):
						props = attributesToProps(domNode.attribs);
						if (!userData.twitter || !socialChecker?.twitter) {
							props.style.display = 'none'
						} else {
							props.style.display = 'inline-block'
						}
					...
					case ('link-instagram-blk'):
						props = attributesToProps(domNode.attribs);
						if (!userData.instagram || !socialChecker?.instagram) {
							props.style.display = 'none'
						} else {
							props.style.display = 'inline-block'
						}
					...
					case ('link-pinterest-blk'):
						props = attributesToProps(domNode.attribs);
						if (!userData.pinterest || !socialChecker?.pinterest) {
							props.style.display = 'none'
						} else {
							props.style.display = 'inline-block'
						}
					...
					case ('link-youtube-blk'):
						props = attributesToProps(domNode.attribs);
						if (!userData.youtube || !socialChecker?.youtube) {
							props.style.display = 'none'
						} else {
							props.style.display = 'inline-block'
						}
					...
					case ('full-name'):
						props = attributesToProps(domNode.attribs);
						return <span {...props}>{userData.fullName}</span>;
					case ('logo-name'):
						props = attributesToProps(domNode.attribs);
						return <span {...props}>{userData.logoName}</span>;
					case ('designation'):
						props = attributesToProps(domNode.attribs);
						return <span {...props}>{userData.designation}</span>;
					case ('mob-no'):
						props = attributesToProps(domNode.attribs);
						return <span {...props}>{userData.mobile}</span>;
					case ('phone'):
						props = attributesToProps(domNode.attribs);
						return <span {...props}>{userData.phone}</span>;
					case ('email'):
						props = attributesToProps(domNode.attribs);
						return <span {...props}>{userData.email}</span>;
					case ('your-addr'):
						props = attributesToProps(domNode.attribs);
						return <span {...props}>{userData.location}</span>;
					case ('website'):
						props = attributesToProps(domNode.attribs);
						return <span {...props}>{userData.website}</span>;
					case ('disclaimer'):
						props = attributesToProps(domNode.attribs);
						return <span {...props}>{userData.disclaimer || domToReact(domNode.children, options)}</span>;
					case ('video-title'):
						props = attributesToProps(domNode.attribs);
					...
					case ('link-whatsapp'):
						if (userData.whatsapp) {
							domNode.attribs.href = userData.whatsapp.startsWith('placeholder') ? '#' : userData.whatsapp;
						}
						props = attributesToProps(domNode.attribs);
						return <a {...props}>{domToReact(domNode.children, options)}</a>
					case ('link-instagram'):
					...
					case ('quote'):
						props = attributesToProps(domNode.attribs);
						return <span {...props}>{userData?.quote ? '"'+userData?.quote+'"' : domToReact(domNode.children, options)}</span>;
					default:
				}
			} catch (error) {
	...
	const ctaCollapseItems = [
			...
			<input
				...
				onChange={(e) => { 
					if(Object.keys(chosenTemplateData).length > 0) { 
						setUserData(prev => ({ ...prev, disclaimer: e.target.value?.slice(0,1000) })); 
		...
		>
			...
			<input
				...
				onChange={(e) => {
					if (Object.keys(chosenTemplateData).length > 0) {
						setUserData(prev => ({ ...prev, quote: e.target.value?.slice(0, 500) }));
	...
	const validateTabberChange = (tabData, editMode) => {
		...
		if(editMode === true){
			...
			if (tabData === 'filter-social') {
				...
				if (elem?.value.length > 1 && elem?.value !== initialValues.fullName) {
					if (!elem?.value.replace(/\s/g, '').length) {
						errorFlag = true;
						setWarning(prev =>({...prev, name:{status:true, message:"Please fill your name"}}));
					}
					else if(elem?.value === "Please fill your name"){
						errorFlag = true;
						setWarning(prev =>({...prev, name:{status:true, message:"Please fill your name"}}))
					} else {
						allow+=1;
				...
				if (designationElem?.value.length > 1 && designationElem?.value !== initialValues.designation) {
					if (!designationElem?.value.replace(/\s/g, '').length) {
						errorFlag = true;
						setWarning(prev =>({...prev, designation:{status:true, message:"Please fill your designation"}}));
					}
					else if(designationElem?.value === "Please fill your designation"){
						errorFlag = true;
						 setWarning(prev =>({...prev, designation:{status:true, message:"Please fill your designation"}}))
					} else {
						allow+=1;
				...
				if (emailElem?.value.length > 1 && emailElem?.value !== initialValues.email) {
					if (!emailElem?.value.replace(/\s/g, '').length) {
						errorFlag = true;
						setWarning(prev =>({...prev, email:{status:true, message:"Please fill your email"}}));
					}
					else if(emailElem?.value === "Please fill your email"){
						errorFlag = true;
						 setWarning(prev =>({...prev, email:{status:true, message:"Please fill your email"}}))
					} else {
						allow+=1;
				...
				if(errorFlag){
					enqueueSnackbar('Please fill all the necessary fields', {
	...
	console.log('##### CHOSEN TEMPLATE DATA #####',chosenTemplateData)
			...
			if (templates.length > 0) {
				...
				if (match) {
					...
					if (loggerData?.isLoggedIn) {
						if (isDraft) {
							setUserData(prev => ({...prev, ...state?.chosen_draft_userData}))
						} else {
							if(Object.values(loggerData?.userData)?.length > 0){
								setUserData(prev => ({...prev, ...loggerData?.userData}))
							} else {
								setUserData(prev => ({ ...match.filler, ...prev  }))
								setInitialValues(prev => ({ ...match.filler, ...prev }))		
							}
						}
					} else {
					...
					if (loggerData?.isLoggedIn) {
						if (isDraft) {
							...
							if(Object.values(loggerData?.userData)?.length > 0){
								...
								setUserData(prev => ({ ...template_data?.data?.data?.filler, ...prev, }))
								setInitialValues(prev => ({ ...template_data?.data?.data?.filler,  ...prev }))
							}
						}
					} else {
						setUserData(prev => ({ ...template_data?.data?.data?.filler,  ...prev }))
	...
	async function handleImageUpload(data) {
		...
		try {
			...
			if (result.status === 200 && result?.data?.success === true) {
				console.log(result);
				setUserData(prev => ({ ...prev, profileImage: result?.data?.image }))
				setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, profileImage: result?.data?.image } }));
	...
	const handleGmailSignature = async () => {
		try {
			if (!loggerData?.isLoggedIn) {
				...
				const node_text = reactDOMServer.renderToString(node);
				const result = await userServices.gmailAuth(node_text, loggerData?.email);
				if (result?.status === 200) {
	...
	async function handleRefresh() {
		try {
			...
			if (log_result.status === 200) {
				...
				setUserData(prev => ({...prev, ...local_user}));
				setLoggerData(prev => ({ ...prev, username: log_result?.data?.username, email: log_result?.data?.email, access_token: log_result?.data?.access_token, user_id: log_result?.data?.user_id, isLoggedIn: true, userData:{...local_user} }));
				fetchChosenTemplate(localStorage.getItem('template_id'), false)
			}
		} catch (error) {
	...
	// 		const dimensions = userData?.profileImage_size?.split(" ")?.join("")?.split("px");
	// 		cropperRef.current.cropper.setCropBoxData({
	// 		  width: parseInt(dimensions[0]), // Fixed width
	// 		  height: parseInt(dimensions[1]), // Fixed height
	// 		});
	// 	}
	// }, [])
					...
					<div className="center-wr">
						<div className="create-template-content clearfix">
							<div className="create-template-left">
								...
								<div className="create-details-box create-box-blk" data-filter="filter-details" onClick={(e) => { validateTabberChange('filter-details', true) }}>
								<div className="create-details-box-count"  
								style={{
									backgroundColor: ((userData?.fullName && userData?.email && userData?.designation) && (userData?.fullName !== initialValues.fullName && userData?.email !== initialValues?.email && userData?.designation !== initialValues?.designation)) && '#71dd37'}}>
										{((userData?.fullName && userData?.email && userData?.designation) && (userData?.fullName !== initialValues.fullName && userData?.email !== initialValues?.email && userData?.designation !== initialValues?.designation)) ? <img src='assets/images/tick-svgrepo-com.svg' alt="" width={24} height={24} /> : '2' }
								</div>
									<div className="create-template-box-inner">
										<div className="create-box-icon">
										...
										<form method="post" action="">
													...
													>
														<div id="drop-zone-container" style={{display:'flex', flexDirection:'column', alignItems:"center", justifyContent:'start', gap:"10px"}}>
															...
															{src && <div style={{width:'100%', padding:'5px 10px', display:'flex', alignItems:'center', justifyContent:'center', gap:'20px', paddingTop:'5px'}}>
																<Button style={{backgroundColor:"#ececec", boxShadow:"2px 2px 12px 0px rgba(0,0,0,0.23)"}} shape="rounded" onClick={()=>{setCircular(true)}} icon={<FaRegCircle color="#484848"/>}></Button>
																<Button style={{backgroundColor:"#ececec", boxShadow:"2px 2px 12px 0px rgba(0,0,0,0.23)"}} shape="rounded" onClick={()=>{setCircular(false)}} icon={<FaRegSquare color="#484848"/>}></Button>
															</div>}
														</div>
													</Modal>
												</label>
											...
											<div className="create-name form-field">
												...
												onFocus={(e) => { if(warning?.name?.status){
												 ...
												 maxLength="40"  
												 value={warning?.name?.status ? warning?.name?.message : userData?.fullName} 
												 required id='name-required' onChange={(e) => { if (Object.keys(chosenTemplateData).length > 0) { setUserData(prev => ({ ...prev, fullName: e.target.value?.slice(0, 40) })); setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, fullName: e.target.value?.slice(0, 40) } })) } }} type="text" name="name" autoComplete="off" />
											</div>
											<div className="create-job-title form-field" style={{display:chosenTemplateData ? chosenTemplateData?.filler?.designation ? "block":"none":"block"}}>
												...
												<input 
													...
													maxLength="30" 
													onChange={(e) => { if (Object.keys(chosenTemplateData).length > 0) { setUserData(prev => ({ ...prev, designation: e.target.value?.slice(0, 30) })); setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, designation: e.target.value?.slice(0, 30) } })) } }} 
													value={warning?.designation?.status ? warning?.designation?.message : userData?.designation} 
											...
											<div className="create-email form-field" style={{display:chosenTemplateData ? chosenTemplateData?.filler?.email ? "block":"none":"block"}}>
												...
												value={warning?.email?.status ? warning?.email?.message : userData?.email}
											...
											<div className="create-phone form-field" style={{display:chosenTemplateData ? chosenTemplateData?.filler?.phone ? "block":"none":"block"}}>
												...
												<input  onFocus={(e) => { e.target.select() }} maxLength="20" onChange={(e) => { if (Object.keys(chosenTemplateData).length > 0) { setUserData(prev => ({ ...prev, phone: e.target.value?.replace(/^[A-Za-z]*$/, "")?.slice(0, 20) })); setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, phone: e.target.value?.replace(/^[A-Za-z]*$/, "")?.slice(0, 20) } })) } }} value={userData?.phone} type="text" name="create-phone" autoComplete="off" />
											</div>
											<div className="create-website form-field" style={{display:chosenTemplateData ? chosenTemplateData?.filler?.website ? "block":"none":"block"}}>
												...
												<input  onFocus={(e) => { e.target.select() }} maxLength="50" onChange={(e) => { if (Object.keys(chosenTemplateData).length > 0) { setUserData(prev => ({ ...prev, website: e.target.value?.slice(0, 50) })); setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, website: e.target.value?.slice(0, 50) } })) } }} value={userData?.website} type="text" name="create-website" autoComplete="off" />
											</div>
											<div className="create-website form-field" style={{display:chosenTemplateData ? chosenTemplateData?.filler?.location ? "block":"none":"block"}}>
												...
												<input  onFocus={(e) => { e.target.select() }} maxLength="100" onChange={(e) => { if (Object.keys(chosenTemplateData).length > 0) { setUserData(prev => ({ ...prev, location: e.target.value?.slice(0, 100) })); setLoggerData(prev => ({ ...prev, userData: { ...prev.userData, location: e.target.value?.slice(0, 100) } })) } }} value={userData?.location} type="text" name="create-address" autoComplete="off" />
											</div>
										</form>
									</div>
									<div className="create-template-details">
										...
										<div className="template-detail-btn">
											...
											<a href="#" onClick={() => { validateTabberChange('filter-social', true) }} style={{float:'right'}} data-filter="filter-social" className="anchor-tabber site-btn-second">Put Social Media</a>
										</div>
									</div>
								</div>
							</div>
								...
								<div className="create-template-right-inner">
									<div className="create-template-categories  animate__animated animate__fadeIn login-form">
										<form method="post" action="">
											...
											<label htmlFor="facebook-check" id="custom-facebook-check"></label>
												...
												<input placeholder="https://www.facebook.com/username"  onFocus={(e) => { e.target.select() }} maxLength="100" value={userData.facebook} className="social-link-check" 
												onChange={(e) => { 
													if (Object.keys(chosenTemplateData).length > 0) { 
											...
											<label htmlFor="instagram-check" id="custom-instagram-check"></label>
												...
												<input placeholder="https://www.instagram.com/username/" onFocus={(e) => { e.target.select() }} maxLength="100" value={userData.instagram} className="social-link-check" 
												onChange={(e) => { 
													if (Object.keys(chosenTemplateData).length > 0) { 
											...
											<div className="create-website form-field contact-form-field checkboxed-input">
												...
												<input placeholder="https://twitter.com/username" onFocus={(e) => { e.target.select() }} maxLength="100" value={userData.twitter} className="social-link-check" 
												onChange={(e) => { 
													if (Object.keys(chosenTemplateData).length > 0) { 
											...
											<div className="create-company form-field contact-form-field checkboxed-input">
												...
												<input placeholder="https://www.linkedin.com/in/username" onFocus={(e) => { e.target.select() }} maxLength="100" value={userData.linkedIn} className="social-link-check" 
												onChange={(e) => { 
													if (Object.keys(chosenTemplateData).length > 0) { 
											...
											<div className="create-phone form-field contact-form-field checkboxed-input">
												...
												<input  onFocus={(e) => { e.target.select() }} maxLength="100" placeholder="https://www.youtube.com/watch?v=VIDEO_ID" value={userData.youtube} className="social-link-check" 
												onChange={(e) => { 
													if (Object.keys(chosenTemplateData).length > 0) { 
											...
											<label htmlFor="skype-check" id="custom-skype-check"></label>
												...
												<input  onFocus={(e) => { e.target.select() }} placeholder="skype:username?chat" maxLength="100" value={userData.skype} className="social-link-check" 
												onChange={(e) => { 
													if (Object.keys(chosenTemplateData).length > 0) { 
											...
											<label htmlFor="whatsapp-check" id="custom-whatsapp-check"></label>
												...
												<input  onFocus={(e) => { e.target.select() }} placeholder="https://wa.me/1234567890" maxLength="100" value={userData.whatsapp} className="social-link-check" 
												onChange={(e) => { 
													if (Object.keys(chosenTemplateData).length > 0) { 
											...
											<label htmlFor="pinterest-check" id="custom-pinterest-check"></label>
												...
												<input  onFocus={(e) => { e.target.select() }} placeholder="https://www.pinterest.com/username/" maxLength="100" value={userData.pinterest} className="social-link-check" 
												onChange={(e) => { 
													if (Object.keys(chosenTemplateData).length > 0) { 
									...
									<div className="create-template-details">
										...
										<div className="template-detail-btn">
											...
											<a href="#" onClick={() => { validateTabberChange('filter-cta', true) }} style={{float:'right'}} data-filter="filter-cta" className="anchor-tabber site-btn-second">Add CTA</a>
										</div>
									</div>
								</div>
							</div>
											...
											<div className="template-detail-first-btn">
												{
													loggerData?.isLoggedIn ?
														<div>
															<a href="#" onClick={() => { validateTabberChange('filter-social', true) }} data-filter="filter-cta" className="anchor-tabber site-btn-second-reverse">Back</a>
															<div onClick={handleSave} style={{marginLeft:'10px', display:'inline-block'}} className="site-btn-second">Save</div>
														</div>
														:
														<div>
															<a href="#" onClick={() => { validateTabberChange('filter-social', true) }} data-filter="filter-cta" className="anchor-tabber site-btn-second-reverse">Back</a>
															<div onClick={loginAndRedirect} style={{marginLeft:'10px', display:'inline-block'}} className="site-btn-second">Save</div>
														</div>
												}
											</div>
